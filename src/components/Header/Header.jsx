...
import { Badge, Box, Button, Grid, Modal } from "@mui/material";
import Notifications from "../UI/Notifications/Notifications";

const navLinksNOtAuth = [
  { path: "/home", display: "الرئيسية" },
  { path: "/about", display: "حول" },
  { path: "/contact", display: "تواصل معنا" },
];

const navLinksAuth = [
  { path: "/home", display: "الرئيسية" },
  { path: "/about", display: "حول" },
  { path: "/cars", display: "سيارات" },
  { path: "/aqar", display: "عقارات" },
  { path: "/other", display: "أخرى" },
  { path: "/all", display: "الكل" },
  { path: "/tender", display: "مناقصة" },
  { path: "/contact", display: "تواصل معنا" },
  { path: "/addauction", display: "إضافة" },
];
...
const Header = () => {
  ...
  const handleConfirm = (path) => {
    navigate(path);
    handleClose(); // إغلاق المودال بعد التنقل
};
...
useEffect(() => {  
  const intervalId = setInterval(async () => {
    if(user.status!==false){
      try {
        const response =await axios.get('http://localhost:8000/api/v1/user/notifications',config)
        const unread=response.data.notifications.filter(notifications=>notifications.status===0)
        ...
        <Container>
          <Row>
            ...
            {user.status == false ? (
              <Col lg="6" md="6" sm="6">
                <Zoom triggerOnce>
                  <div className="header__top__right d-flex align-items-center justify-content-end gap-1">
                  ...
                  <div className="menu">
                    ...
                    >
                      <Badge badgeContent={unreadCount} color="error">
                        <NotificationsIcon fontSize="medium" color="fff" />
                      </Badge>
                    </NavLink>
                  </div>
                </Zoom>
