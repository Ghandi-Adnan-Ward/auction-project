...
const AqarDetails1 = (props) => {
  const {name,minimum_bid ,end_time,details,image,id ,status,description,current_bid,type,increment_amount} =  props.AqarData ;
...
const formatTime = (totalSeconds) => {
  ...
  const url1='http://localhost:8000/api/v1/user/live-auctions/'+id+'/bid';
  const url2='http://localhost:8000/api/v1/user/regular-auctions/'+id+'/bid'
  ...
  const handleBidSubmit = (event) => {
    ...
    bbid.append('bid_amount',event?.target.bid_amount.value)
      ...
      try {
        ...
        axios.post(url1,current_bid,config)
       .then(res=>{
    ...
    <section>
      <Container>
        <Row>
          <Col sm='6' md='6' lg="6">
            <Roll>
            <img src={`http://localhost:8000/storage/${image}`} alt="aqar" className="w-75 h-75" />
