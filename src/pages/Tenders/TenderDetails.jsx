...
const TenderDetails = (props) => {
  const {name,minimum_bid ,end_time,image,id ,status,description,current_bid,type,increment_amount,model_link} =  props.Data ;
...
const formatTime = (totalSeconds) => {
  ...
  const url='http://localhost:8000/api/v1/user/decreasing-auctions/'+id+'/bid'
...
const handleBidSubmit = (event) => {
  ...
  const bbid=new FormData();
    ...
    try {
      ...
      axios.post(url,current_bid,config)
     .then(res=>{
      ...
      <section>
        <Container>
          ...
          <div className="form m-4">
            <h1 className="section__title">ادخل قيمة المناقصة</h1>
            {type!='live' ? 
            <TextField 
            ...
            <br />
            <Button type="submit"  className="mt-2 p-2" variant="contained" color="primary">مناقصة</Button>
               <br />
                <p>الوقت المتبقي لانتهاء المناقصة : {formatTime(remaing)}</p>
                <br />
            </div>
        </form>:
          (
            <div>
                 <p className="section__subtitle text-center">المناقصة انتهت</p>
                 <p className="section__subtitle1 text-center">{}</p>
                 <p>الوقت المتبقي لانتهاء المناقصة : {formatTime(remaing)}</p>
                  <p>{currentTime}</p>
                  <p>{end_time}</p>
                 <p className="section__title text-center">رابح المناقصة: {WinnerData} {}</p>
               </div>
       )
        }
            </Zoom>
